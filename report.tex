...
\author{230910131\quad 周宇轩 \\ \small 计算机视觉课程报告}
\date{\today}
...
\end{equation}
其中$f_{\text{Swin}}(x)$为Swin Transformer提取的768维特征向量。识别模型的PyTorch实现详见附录~\ref{code:recognizer}。
...
\end{equation}
其中$p_t$为正确类别的预测概率，$\gamma$为聚焦参数。我们设置$\gamma = 2.0$，使得预测概率为0.9的样本损失权重仅为0.01，而预测概率为0.5的样本损失权重为0.25。Focal Loss的PyTorch实现详见附录~\ref{code:focal}。
...
\end{equation}
其中$\alpha = 0.5$，$P_{\text{cb}}$为类别均衡概率，$P_{\text{ib}}$为实例均衡概率。混合采样器的实现详见附录~\ref{code:sampler}。
...
推理脚本支持批量识别（batch size=64），在A100 GPU上可达到约50张图像/秒的处理速度。Dockerfile详见附录~\ref{code:dockerfile}。
...
\begin{lstlisting}[caption={识别模型定义}]
class Recognizer(nn.Module):
    def __init__(self, num_classes, backbone='swin_small',
        ...
        if backbone == 'swin_small':
            ...
            self.backbone = models.convnext_s(
...
\subsection{Focal Loss实现}
\label{code:focal}

\begin{lstlisting}[caption={Focal Loss实现}]
...
\begin{lstlisting}[caption={混合采样器实现}]
class HybridSampler(Sampler):
    def __init__(self, labels, alpha=0.5):
        self.labels = np.array(labels)
        self.alpha = alpha
        class_counts = np.bincount(self.labels)
        # Class-balanced: uniform per class
        self.cb_weights = 1.0 / class_counts[self.labels]
        self.cb_weights /= self.cb_weights.sum()
        # Instance-balanced: uniform per sample
        self.ib_weights = np.ones(len(labels)) / len(labels)

    def __iter__(self):
        probs = (self.alpha * self.cb_weights
                 + (1 - self.alpha) * self.ib_weights)
...
\begin{lstlisting}[caption={Dockerfile},language=bash]
FROM nvidia/cuda:12.4.0-runtime-ubuntu22.04
ENV TZ=Asia/Shanghai LANG=C.UTF-8 PYTHONUNBUFFERED=1
RUN apt-get update && apt-get install -y \
    python3 python3-pip libgl1 libgomp1 && \
...
\begin{lstlisting}[caption={端到端推理流程}]
def run_inference(image_dir, det_model, rec_model,
                  mapping, conf_threshold=0.15):
    results = {}
    for img_path in sorted(Path(image_dir).glob("*.png")):
        # Step 1: Detection with YOLOv8s-OBB
        det_result = det_model.predict(
            str(img_path), conf=conf_threshold)
        boxes = det_result[0].obb

        # Step 2: Crop detected characters
        crops = []
        for box in boxes:
        ...
        if crops:
            ...
            with torch.no_grad():
                logits = rec_model(batch)
                preds = logits.argmax(dim=1).cpu().tolist()
            chars = [mapping[p] for p in preds]
            results[img_path.name] = chars
    return results
\end{lstlisting}
